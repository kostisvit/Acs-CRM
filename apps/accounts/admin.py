...
class CustomUserAdmin(ImportExportModelAdmin):
    ...
    list_display = ('id','email','display_company', 'first_name', 'last_name','sex_choice','job_title', 'is_staff', 'is_active','created','modified')
    list_filter = ('is_staff', 'is_active')
    readonly_fields = ()
    fieldsets = (
        (None, {'fields': ('email','company','job_title','days_sum','days_left')}),
        ('Personal info', {'fields': ('first_name', 'last_name', 'phone_number','sex_choice',)}),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('first_name', 'last_name','sex_choice', 'email','job_title','days_sum','days_left','is_superuser','is_staff','is_active','phone_number','groups', 'user_permissions')}
        ),
    )
    search_fields = ('email',)
    ordering = ('email',)
